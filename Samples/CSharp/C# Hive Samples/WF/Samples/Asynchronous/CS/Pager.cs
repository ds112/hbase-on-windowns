...
namespace Asynchronous
{
    ...
    {
        int _currentPage;
        private HiveResultSet ResultSet;
        ...
        private void FillPage(HiveResultSet table)
        {
            DisplayPageInfo();
            ...
            _grid.TableDescriptor.Columns["contactid"].HeaderText = "Contact Id";
            _grid.TableDescriptor.Columns["fullname"].HeaderText = "Full Name";
            _grid.TableDescriptor.Columns["age"].HeaderText = "Age";
            _grid.TableDescriptor.Columns["emailaddress"].HeaderText = "Email Address";
            _grid.TableDescriptor.Columns["phoneno"].HeaderText = "Phone No";
            _grid.TableDescriptor.Columns["modifieddate"].HeaderText = "Modified Date";
            _grid.TableDescriptor.Columns["contactid"].Width = 85;
            _grid.TableDescriptor.Columns["fullname"].Width = 170;
            _grid.TableDescriptor.Columns["age"].Width = 90;
            _grid.TableDescriptor.Columns["emailaddress"].Width = 230;
            _grid.TableDescriptor.Columns["phoneno"].Width = 170;
            _grid.TableDescriptor.Columns["modifieddate"].Width = 170;
        ...
        private void DisplayPageInfo()
        {
            ...
            if (Reader.HasRows || resultCollection.ContainsKey(_currentPage + 1))
        ...
        private Dictionary<int, HiveResultSet> resultCollection = new Dictionary<int, HiveResultSet>();
        /// <summary>
        ...
        async void RecordNavigationBar_ArrowButtonClicked(object sender, ArrowButtonEventArgs e)
        {
            switch (e.Arrow)
            {
                ...
                case ArrowType.Next:
                    {
                        _currentPage += 1;
                        if (!Reader.HasRows && !resultCollection.ContainsKey(_currentPage))
                        {
                            _currentPage -= 1;
                        }
                        else if (!Reader.HasRows && resultCollection.ContainsKey(_currentPage))
                        {
                            ResultSet = resultCollection[_currentPage];
                        }
                        else
                        {
                            if (Reader.HasRows)
                            {
                                ResultSet = await Reader.FetchResultAsync();
                            }
                        }
