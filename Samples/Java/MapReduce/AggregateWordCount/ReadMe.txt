...
Pre-requisites to submit jobs in Secure Cluster through Big Data Command Shell form the BigData Dashboard.
 ...
 5. Replace the following line [41] into the "build.gradle" file.
		commandLine 'cmd', '/c', "$syncfusionHadoopSDKPath/bin/hadoop --config <Secured-hadoop-conf-directory> jar $projectDir/build/libs/AggregateWordCount.jar ", args[0]
...
To build : 
	gradle -p %SYNCBDP_HOME%\Samples\Java\MapReduce\AggregateWordCount build
To run : 
	gradle -p %SYNCBDP_HOME%\Samples\Java\MapReduce\AggregateWordCount runJar -Parguments="hostname"
