...
public class ExampleDriver {
  ...
  public static void main(String argv[]){
	  ...
	  /*ExampleDriver ed=new ExampleDriver();	
	  Configuration conf=new Configuration();
	  conf.addResource(argv[argv.length-1]+"/etc/hadoop");
	  ed.setConf(conf);
	  String argTemp[]=argv;
	  argv = new String[argTemp.length-2];
	  for(int i=0;i<argTemp.length-1;i++)
		  if(i!=0)
			  argv[i-1]=argTemp[i];
	  System.out.println("kaveen2: "+argv[0]);*/
    int exitCode = -1;
